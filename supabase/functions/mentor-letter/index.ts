...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `אתה מנטור פדגוגי מנוסה וחם הכותב מכתב אישי ומעצים ל${userData?.gender === 'female' ? 'מנהלת' : 'מנהל'} פסג"ה.
...
- בעברית תקנית וזורמת ללא שגיאות כתיב
- השתמש בלשון ${userData?.gender === 'female' ? 'נקבה בכל הפניות (את, עשית, הצלחת, וכו\')' : 'זכר בכל הפניות (אתה, עשית, הצלחת, וכו\')'}

עיצוב המכתב (חשוב מאוד!):
- כל משפט יסתיים בנקודה ואחריו שורה חדשה
- חלק את המכתב לפסקאות ברורות עם שורה ריקה ביניהן
- כל משפט יהיה קצר וממוקד (עד 12 מילים)
- הדגש מילות מפתח חשובות עם **הדגשה** - לא יותר מ-2 הדגשות בפסקה
- אל תשתמש בסימני * בודדים, רק **הדגשה** כפולה

אורך המכתב: 180-250 מילים, מחולק ל-5-6 פסקאות קצרות`;
